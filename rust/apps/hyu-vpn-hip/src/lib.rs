...
pub fn build_hip_from_args(
    ...
    if let Some(value) = &client_ip {
        if !matches!(value.parse::<IpAddr>(), Ok(IpAddr::V4(_))) {
            return Err(HipCliError::InvalidInvocation);
        }
    }
    if let Some(value) = &client_ipv6 {
        if !matches!(value.parse::<IpAddr>(), Ok(IpAddr::V6(_))) {
            return Err(HipCliError::InvalidInvocation);
        }
    }
    ...
    if let Some(computer) = identity.get("computer").filter(|value| !value.is_empty()) {
        posture.hostname = computer.clone();
    }

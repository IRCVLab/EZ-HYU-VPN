...
한양대학교 VPN을 macOS 메뉴바에서 간단하게 연결하는 앱입니다. OpenConnect 기반으로 동작하며, VPN 연결·해제와 OTP 확인을 한곳에서 처리합니다.
...
- 메뉴바의 **V 아이콘**을 눌러 VPN을 연결하거나 해제합니다.
- OTP 항목에는 현재 코드와 남은 시간이 표시됩니다.
- OTP 항목을 누르면 코드가 클립보드에 복사됩니다.
- **Launch at Login**으로 로그인 시 자동 실행을 설정할 수 있습니다.
- 새 버전이 나오면 메뉴바와 알림창에서 다운로드 페이지를 안내합니다. 업데이트 확인 실패는 VPN 연결에 영향을 주지 않습니다.
- 앱을 종료하려면 **Quit HYU VPN**을 선택합니다.

v0.1.0에는 업데이트 알림 기능이 없으므로 v0.1.1로 한 번 직접 업데이트해야 합니다. v0.1.1부터는 이후 버전을 앱에서 알려줍니다.

...
rm -rf "$app_dir"
mkdir -p "$macos_dir" "$resources_out"

/usr/bin/swiftc "$source_file" -o "$macos_dir/$executable_name"
chmod 0755 "$macos_dir/$executable_name"

if [ -d "$resources_dir" ]; then
  (cd "$resources_dir" && /usr/bin/find . -type f ! -name '.DS_Store' -print | while IFS= read -r item; do
    target="$resources_out/${item#./}"
    mkdir -p "$(dirname "$target")"
    cp "$resources_dir/${item#./}" "$target"
  done)
fi

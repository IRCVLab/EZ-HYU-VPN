...
phase_install_identity(){ run_installer_app "$MOUNTPOINT/Install HYU VPN.app" >/dev/null; verify_installed_file "$(map_installed '/Library/Application Support/HYU VPN/bin/hyu-vpn-macos-service')" hyu-vpn-macos-service 755 service; verify_installed_file "$(map_installed /Library/PrivilegedHelperTools/com.hyu.vpn.helper)" com.hyu.vpn.helper 755 helper; verify_installed_file "$(map_installed '/Applications/HYU VPN.app/Contents/MacOS/HYUVPNMenuApp')" 'HYU VPN.app/Contents/MacOS/HYUVPNMenuApp' 755 menu; verify_launchd_plist "$HOME/Library/LaunchAgents/com.hyu.vpn.service.plist"; validate_backend_status "$(ipc_request status)"; printf 'phase=install_identity result=verified backend_version=0.2.3\n'; }

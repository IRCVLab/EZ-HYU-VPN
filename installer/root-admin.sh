...
APP_SUPPORT="$(map_path '/Library/Application Support/HYU VPN')"; HELPER_DST="$(map_path /Library/PrivilegedHelperTools/com.hyu.vpn.helper)"; SUDOERS_DST="$(map_path /etc/sudoers.d/com.hyu.vpn)"; APP_DST="$(map_path '/Applications/HYU VPN.app')"
...
remove_rel(){ local rel="$1" target; case "$rel" in /*|*..*|Library/Preferences/SystemConfiguration*) print -u2 "unsafe installed path: $rel"; return 1;; esac; case "$rel" in Library/Application\ Support/HYU\ VPN/*|Library/PrivilegedHelperTools/com.hyu.vpn.helper|Applications/HYU\ VPN.app|etc/sudoers.d/com.hyu.vpn|Users/*/Library/LaunchAgents/com.hyu.vpn.*.plist|private/var/db/hyu-vpn/*) ;; *) print -u2 "unsafe installed path: $rel"; return 1;; esac; [[ "$rel" == /* ]] && target="$(map_path "$rel")" || target="$ROOT_PREFIX/$rel"; [[ -e "$target" || -L "$target" ]] || return 0; [[ -d "$target" && ! -L "$target" ]] && /bin/rm -rf "$target" || /bin/rm -f "$target"; }
...
install_phase(){
  ...
  run_cmd /usr/sbin/chown -R root:wheel "$APP_SUPPORT" "$HELPER_DST" "$APP_DST" "$STATE_DIR"
  ...
  OC_ABS="/Library/Application Support/HYU VPN/runtime/current/bin/openconnect"; VPNC_ABS="/Library/Application Support/HYU VPN/runtime/vpnc/hyu-vpnc-wrapper"; HIP_ABS="/Library/Application Support/HYU VPN/runtime/gp-hip-report"; OC_HASH="$(sha256 "$RUNTIME_DIR/bin/openconnect")"; VPNC_HASH="$(sha256 "$APP_SUPPORT/runtime/vpnc/hyu-vpnc-wrapper")"; HIP_HASH="$(sha256 "$APP_SUPPORT/runtime/gp-hip-report")"; cfg="{\"openConnectExecutable\":\"$OC_ABS\",\"vpncScript\":\"$VPNC_ABS\",\"hipWrapper\":\"$HIP_ABS\",\"stateDirectory\":\"/private/var/db/hyu-vpn\",\"ledgerDirectory\":\"/private/var/db/hyu-vpn/ledger\",\"openConnectExecutableSHA256\":\"$OC_HASH\",\"vpncScriptSHA256\":\"$VPNC_HASH\",\"hipWrapperSHA256\":\"$HIP_HASH\"}"
  write_file "$APP_SUPPORT/helper-config.json" 600 "$cfg"; fail_after app
  sudo_tmp="$(/usr/bin/dirname "$SUDOERS_DST")/com.hyu.vpn.tmp.$$"; /bin/mkdir -p "$(/usr/bin/dirname "$SUDOERS_DST")"; print -- "$ADMIN_USER ALL=(root) NOPASSWD: /Library/PrivilegedHelperTools/com.hyu.vpn.helper start, /Library/PrivilegedHelperTools/com.hyu.vpn.helper stop, /Library/PrivilegedHelperTools/com.hyu.vpn.helper status, /Library/PrivilegedHelperTools/com.hyu.vpn.helper repair" > "$sudo_tmp"; /bin/chmod 440 "$sudo_tmp"; run_cmd /usr/sbin/visudo -c -f "$sudo_tmp"; backup_target "$SUDOERS_DST"; run_cmd /bin/mv "$sudo_tmp" "$SUDOERS_DST"; [[ -e "$sudo_tmp" ]] && /bin/mv "$sudo_tmp" "$SUDOERS_DST"; /bin/chmod 440 "$SUDOERS_DST"; record_path "$SUDOERS_DST"; run_cmd /usr/sbin/visudo -c -f "$SUDOERS_DST"; fail_after sudoers
  run_cmd /usr/bin/env -i PATH=/usr/bin:/bin SUDO_UID="$ADMIN_UID" /usr/bin/python3 "$APP_SUPPORT/bin/hyu-vpn-native-client" suppress-auto-launch; validate_native_snapshot; print native_suppressed >| "$STATE_DIR/native-suppression-transaction"; durable_flush "$STATE_DIR/native-suppression-transaction"; fail_after native-suppression
